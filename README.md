...
Define dependencies between packages in `dependencies` section of the `package.json` for each module.

```json
  "dependencies": {
    "@appgenerator/views": "*",
    "@appgenerator/models": "*",
    "@appgenerator/controllers": "*"
  },
```
...
Each module must be registered in the global (base) `tsconfig.json` under `references`
